 ...
 * 
 */
public class FWImpAction extends BaseBean implements Action {
	public String execute(RequestInfo request) {
		...
		imp.setComments(comments); //.replace("<br>", " ").replace("&nbsp", " ").replace("<", " ").replace(">", " ").replace("&", " ")
		...
		if (detailtable.length > 0) {
			...
			for (int i = 0; i < detailtable.length; i++) {
				...
				for (int j = 0; j < s.length; j++) {
					...
					data.setComments(comments_dt);//.replace("<br>", " ").replace("&nbsp", " ").replace("<", " ").replace(">", " ").replace("&", " ")
					...
					data.setItem_desc(wlsm);//.replace("<br>", " ").replace("&nbsp", " ").replace("<", " ").replace(">", " ").replace("&", " ")
		...
		if("E".equals(ret)){
			...
			request.getRequestManager().setMessagecontent("服务数据写入异常,请通知管理员@!!!");
